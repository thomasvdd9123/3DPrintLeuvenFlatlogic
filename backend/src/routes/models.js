...
router.post(
  '/bulk-import',
  wrapAsync(async (req, res) => {
    await ModelsService.bulkImport(req, res, true, req.headers.referer);
    const payload = true;
    res.status(200).send(payload);
  }),
);
...
router.get(
  '/count',
  wrapAsync(async (req, res) => {
    const payload = await ModelsDBApi.findAll(req.query, { countOnly: true });
...
router.get(
  '/:id',
  wrapAsync(async (req, res) => {
    const payload = await ModelsDBApi.findBy({ id: req.params.id });
