...
import actions from 'actions/models/modelsListActions';
import actionsModels from 'actions/models/modelsFormActions';
...
const ModelsTable = () => {
  ...
  const count = useSelector((store) => store.models.list.count);
  const modalOpen = useSelector((store) => store.models.list.modalOpen);
  const rows = useSelector((store) => store.models.list.rows);
  const idToDelete = useSelector((store) => store.models.list.idToDelete);
  ...
  const update = (id, data) => {
    dispatch(actionsModels.doUpdate(id, data));
  };
  ...
  const getModelsCSV = async () => {
    const response = await axios({
      url: '/models?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
  ...
  return (
    <div>
      <Widget title={humanize('Models')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/models/new' className={classes.element}>
