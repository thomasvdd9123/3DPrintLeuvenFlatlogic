...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/permissions/autocomplete`, { params });
  return response.data;
}

const PermissionsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.permissions.hasPermissionToCreate,
});

export default connect(select)(PermissionsSelectItem);
