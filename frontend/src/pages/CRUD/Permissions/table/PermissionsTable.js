...
import * as dataFormat from 'pages/CRUD/Permissions/table/PermissionsDataFormatters';

import actions from 'actions/permissions/permissionsListActions';
import actionsPermissions from 'actions/permissions/permissionsFormActions';
...
const PermissionsTable = () => {
  ...
  const [filters, setFilters] = useState([{ label: 'Name', title: 'name' }]);
  ...
  const count = useSelector((store) => store.permissions.list.count);
  const modalOpen = useSelector((store) => store.permissions.list.modalOpen);
  const rows = useSelector((store) => store.permissions.list.rows);
  const idToDelete = useSelector((store) => store.permissions.list.idToDelete);
  ...
  const update = (id, data) => {
    dispatch(actionsPermissions.doUpdate(id, data));
  };
  ...
  const sendPermissionsCSV = async () => {
  ...
  return (
    <div>
      <Widget title={humanize('Permissions')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/permissions/new' className={classes.element}>
