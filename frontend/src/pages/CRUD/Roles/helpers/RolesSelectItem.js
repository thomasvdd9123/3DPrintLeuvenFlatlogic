...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/roles/autocomplete`, { params });
  return response.data;
}

const RolesSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.roles.hasPermissionToCreate,
});

export default connect(select)(RolesSelectItem);
