...
import actions from 'actions/roles/rolesListActions';
import actionsRoles from 'actions/roles/rolesFormActions';
...
const RolesTable = () => {
  ...
  const count = useSelector((store) => store.roles.list.count);
  const modalOpen = useSelector((store) => store.roles.list.modalOpen);
  const rows = useSelector((store) => store.roles.list.rows);
  const idToDelete = useSelector((store) => store.roles.list.idToDelete);
  ...
  const update = (id, data) => {
    dispatch(actionsRoles.doUpdate(id, data));
  };
  ...
  const columns = [
...

    {
      ...
      sortable: false,
      renderCell: (params) =>
        permissionsDataFormat.listFormatter(
      ...
      valueFormatter: ({ value }) =>
        dataFormat.permissionsManyListFormatter(value).join(', '),
  ...
  return (
    <div>
      <Widget title={humanize('Roles')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/roles/new' className={classes.element}>
